...
// 网站的根目录
const char* doc_root = "/home/tom-code/LinuxRepositories/testWebServer/resources";
...
// add the file descriptor to be listened to in epoll
...
http_conn::HTTP_CODE http_conn::parse_content(char* text){
    if(m_read_idx >= (m_content_length + m_checked_idx)){   // why ?
...
http_conn::HTTP_CODE http_conn::process_read(){
    ...
    char* text = 0;
    printf("I'm being in here!\n");
    while(((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK)) ||((line_status = parse_line()) == LINE_OK)){
        text = get_line();
        m_start_line = m_read_idx;
        printf("get 1 http line: %s", text);
